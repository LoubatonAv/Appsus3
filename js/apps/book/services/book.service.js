...
function getGoogleResults(search) {
  return axios
    .get(
      `https://www.googleapis.com/books/v1/volumes?printType=books&q=${search}`
    )
    .then((res) => {
      _saveGoogleBooksToStorage(res.data.items);
...
function addBook(googleBook) {
  const books = _loadBooksFromStorage();
  const { title, subtitle } = googleBook.volumeInfo;
  var newBook = {
...
function _saveGoogleBooksToStorage(googleBooks) {
  storageService.saveToStorage(RESULT_KEY, googleBooks);
}

function _loadBooksFromGoogleStorage() {
  return storageService.loadFromStorage(RESULT_KEY);
}
